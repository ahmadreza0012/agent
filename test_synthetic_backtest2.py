...
Uses synthetic data with more hours to ensure rebalancing occurs.
"""
import numpy as np
import pandas as pd
...
np.random.seed(42)
n_hours = 500  # More hours to ensure multiple rebalances
n_assets = 6
...
try:
    ...
    if 'folds' in results:
        ...
        for i, fold in enumerate(results['folds']):
            ...
            # Check rebalance events in each fold
            if 'rebalance_events' in fold and len(fold['rebalance_events']) > 0:
                print(f"    Rebalance events: {len(fold['rebalance_events'])}")
                last_event = fold['rebalance_events'][-1]
                if 'individual_weights' in last_event and last_event['individual_weights']:
                    print(f"    Individual weights keys: {list(last_event['individual_weights'].keys())}")
    ...
    if backtester.strategy_realized_performance:
        ...
        print("  (No performance recorded - may need more rebalancing events)")
