"""Debug test to verify hypothetical performance calculation"""
import numpy as np
import pandas as pd
...
print("\nRunning single fold test to debug hypothetical calculation...")

# Manually run a single fold to see what's happening
test_start_idx = int(len(df_prices) * 0.7)
...
if len(result['rebalance_events']) > 0:
    ...
    print(f"Last event keys: {last_event.keys()}")
    if 'individual_weights' in last_event and last_event['individual_weights']:
        print(f"Individual strategies: {list(last_event['individual_weights'].keys())}")
        for name, w in last_event['individual_weights'].items():
            print(f"  {name}: weights shape={w.shape}, sum={w.sum():.4f}")
